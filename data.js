...
function dLog() {
  ...
  S.logs.slice(0, 60).forEach(function(e, idx) {
    ...
    h += '<div class="cb"><div class="ct">' + fd(e.date) + (hasRoute ? ' <span class="pill-route">ROUTE</span>' : '') + '</div>';
  ...
  if (geo.waypoints && geo.waypoints.length) {
    geo.waypoints.forEach(function(w) {
      var mk = L.marker([w.lat, w.lng], {
        icon: mkWaypointIcon(S.waypoints.length + 1),
        zIndexOffset: 900
      }).addTo(map).bindPopup(w.label || "Waypoint");
      S.waypoints.push({lat: w.lat, lng: w.lng, label: w.label || "Waypoint", marker: mk});
    });
  }
