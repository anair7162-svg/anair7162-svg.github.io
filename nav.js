...
function stopNav() {
  ...
  if (elapsed >= 1) {
    S._pendingLogRoute = completedRoute || null;
    editingLogIdx = -1;
    $("ml-dt").value = new Date().toISOString().split("T")[0];
    $("ml-du").value = elapsed;
    $("ml-km").value = S.drivenKm > 0 ? S.drivenKm.toFixed(1) : (completedRoute ? completedRoute.distance.toFixed(1) : "");
...
(function() {
  var ov = $("mgm");
  if (!ov) return;
  var closeModal = function() { ov.classList.remove("on"); };
  var gmBtn = $("mgm-gm");
  var navBtn = $("mgm-nav");
  var okBtn = $("mgm-ok");
  if (gmBtn) gmBtn.addEventListener("click", function() {
    closeModal();
    if (typeof expGM === "function") {
      ...
      expGM();
      toast("Route opened in Google Maps", {tone: "ok", icon: "navigation"});
    }
  });
  if (navBtn) navBtn.addEventListener("click", function() {
    closeModal();
    setTimeout(startNav, 120);
  });
...
$("nlog").addEventListener("click", function() {
  var elapsed = S.navStart ? Math.round((Date.now() - S.navStart) / 60000) : 0;
  S._pendingLogRoute = S.curRoute || null;
  editingLogIdx = -1;
  $("ml-dt").value = new Date().toISOString().split("T")[0];
